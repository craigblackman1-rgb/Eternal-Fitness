...
ASK FIRST:
- Whether Esther's ICO registration (Craig said 2026-08-07 he'd register her) is actually done before treating the ROPA/processor-register content as final — the documents can be drafted regardless, but note the registration status rather than assuming it.
- How to describe Craig/Decoded Ops's own developer access to the production database (including client health records) in the processor register — this is a real, currently-undisclosed processor relationship (Claude flagged it in chat 2026-08-07) and the exact wording/scope of that disclosure is Craig's call, not something to invent.
- Whether Craig now has (or is getting) his own hub login — affects the "who can access client data" section of the internal data-handling policy; as of the last confirmed check only Esther had one.
