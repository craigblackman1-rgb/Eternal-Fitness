...
**Verification gap:** both of the above are additive (new button + new API route, no existing code paths changed) and type-check clean, but have not been clicked through in the real hub — this session has no `/hub` login credentials. Needs a real pass by Craig or Esther before being called fully done per the project's "verify before done" rule.
