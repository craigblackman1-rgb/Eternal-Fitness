...
> ```
> Registry id: `wo-eternalfitness-marketing-hub-followups-2026-08-07`
...
- [x] Bank Transactions — CORRECTED TWICE 2026-08-07, now actually rebuilt to match the mockup. First pass wrongly called this "no rebuild needed" — found the categorization UI existed one click deeper (`/cashflow/transactions/[id]`) and stopped there without checking whether the *list page itself* matched `hub-cashflow-transactions.html`. It didn't: the mockup has no "Import history" file list or per-import detail page at all — it's one unified page showing every transaction directly with inline category dropdowns. Craig caught this ("this page didn't get design updated") and, given 3 options, picked a full rebuild to match the mockup exactly. Rebuilt: new `GET /api/cashflow/transactions` (flat list, all imports), `page.tsx` dropped the import-history section and "View import log" button entirely, search/status-filter now operate on the unified transaction list via the existing `TransactionTable` component (fixed to sync its rows when the parent re-filters — it was previously an uncontrolled snapshot from first mount). The `/cashflow/transactions/[id]` route is left in place but now unlinked, matching this project's precedent for retired routes (`/parq`). Pushed `30e78c5`. **Lesson for next time: "the feature exists somewhere" is not the same claim as "this page matches its mockup" — check both.** Not click-through verified (no hub login credentials this session).
...
- [x] **Session Roller — LIVE-VERIFIED 2026-08-07 via a disposable hub login** (Craig authorised creating throwaway staff accounts). "Roll Over Previous Session" button in `SessionEditor.tsx`, backed by `GET /api/clients/[id]/sessions/latest-completed` — finds the client's most recently *completed* session across all blocks (ordered by `data.session_log.completed_at`, not row order), pulls its exercises/sets/reps/tempo/cues into the session being edited. **Real bug caught and fixed**: the route param is the client-facing `client_number` (e.g. "19"), not `clients.id` — the query tried to bind it directly as a uuid, threw a 500. Fixed by resolving `client_number` → `clients.id` first (same pattern as `generate-block/route.ts`). Verified via a real test session on the designated QA client (`Test - Home Training`, client #19): marked it complete, fetched the endpoint directly, got back the correct exercise/sets/reps/tempo/cue data for both studio and home versions. Empty-state path (no prior completed session) also verified — no crash, session left untouched. Pushed `bc1c149` → bug found → fixed and pushed `a68020b`.
- [x] **Volume Skeletons — LIVE-VERIFIED 2026-08-07.** "Skeleton" dropdown next to "Add exercise to {section}" with 4 presets (Elite Strength 4×6, Hypertrophy 3×10, Endurance/Flow 3×15, Power 3×5). Verified end-to-end live: opened the dropdown (all 4 presets render correctly), picked Hypertrophy, added "3D Lunge" — the new row showed Sets=3/Reps=10/Tempo=2-0-2/Rest=60s exactly, not the old hardcoded 2/10/Controlled/45s default. Pushed `bc1c149`, no bugs found.
- [x] **Bank Transactions filter/edit interaction — real bug caught by the same live-testing pass, unrelated to Lane E but found while logged in.** Editing a transaction's category then changing the search/status filter silently reverted the edit — `TransactionTable`'s optimistic local state was never propagated to the parent's `transactions` array, so a re-filter passed a stale copy back down. Fixed with an `onRowUpdated` callback so the parent stays in sync. Verified live: categorized two demo rows, filtered to "Categorised", both correctly stayed categorized (previously would have shown 0 results). Pushed `a68020b`.
- [x] **Template → client assign action** (2026-08-07, code complete, NOT click-through verified — needs a configured AI provider to fully exercise, out of scope for this pass): investigated the real architecture first — there is NO manual "create a new block" or "add a session" path at all; every block/session is generated exclusively via the AI Plan Agent chat (`generateBlock()` → `/api/claude/generate-block`). Presented Craig 3 options (surface the existing session-level "Apply template" more prominently / seed a 1-session block directly from a template, bypassing AI chat / feed the template into the AI generation prompt as structural grounding). **Craig chose option 3.** Built: `PlanAgentTab.tsx` gets a "Use a template as the framework" picker (clearable badge, sent as `templateId`); `/api/claude/generate-block` fetches the template and threads it through `generateViaAi` as a new `TemplateFramework`; new `buildTemplateFrameworkSection()` in `lib/planAgentPrompt.ts` describes the template's structure/volume in the AI system prompt as a shape guide for every session in the block, while exercise choice still comes from the EXERCISE LIBRARY and this client's own constraints (safety always overrides matching the template); the non-AI fallback generator also honours a template when present. Template `usage_count` increments on successful block creation. Pushed `f90d677`.
- **Also caught while merging in other sessions' concurrent pushes:** a pre-existing tsc error from an unrelated commit (`81293ad`, PT page image cleanup) that had left main briefly broken — `ProcessFlow` still read `image`/`imageAlt` fields that commit had removed from the `steps` array. Fixed immediately, pushed `f6bc609`.
- **Worktree note for next session:** this worktree had no `.env.local`, so the local dev server's DB connection silently fell back to the wrong port (5432 instead of the SSH-tunnel 5433) until copied in from the shared checkout — copied now (gitignored, not committed), should persist for future sessions using this worktree.
