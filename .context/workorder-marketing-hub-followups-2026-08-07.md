...
- [ ] Blind/visually-impaired + cancer-rehab specialist copy (British Blind Sport Activity Finder listing, Roy Turnham workshop mentoring, CanRehab cancer rehab) reconciled into the site — Esther supplied two draft variants (a short standalone block and a longer "Specialist Rehabilitation & Inclusive Fitness" block); needs a decision on which page(s) and whether it triggers the roll-call exception
...
- [GATE] Confirm with Craig/Esther which of the two supplied copy variants (short vs. "Specialist Rehabilitation & Inclusive Fitness" long block) goes live, and on which page(s) — both reference named conditions, which needs the same explicit override Craig gave for the existing Specialist Training section
- [AUTO] Once confirmed: implement copy, cross-check against the "Level 4 = CanRehab, not 'Level 4 PT'" rule — files: TBD per placement decision — VERIFY: rendered page read back against approved copy, no roll-call regression elsewhere on the page
