 **Corrected to built, 2026-08-18** — delivered in `bb927e1` (2026-08-17): mobile `TasksPanel` (list + status-toggle + quick-add + delete) reusing the existing `/api/tasks` endpoints, rendered on `/hub/m/clients/[id]`. No schema change needed. Confirmed present on this branch. | **Corrected to built, 2026-08-18** — delivered in `4191575` (2026-08-17): `clients.start_date` DATE column (migration `20260817_client_start_date.sql`, applied to prod, nullable so existing clients stay blank until backfilled), field on the client edit form, shown on both desktop Snapshot and PWA profile Contact panel. Confirmed present on this branch. | **Corrected to built, 2026-08-18** — delivered in `4191575` (2026-08-17): pencil button on each exercise card in the Train Screen linking into the existing edit sheet. Craig's note in the same commit: functional-first, designs still in progress with the client. Confirmed present on this branch. | **Corrected to built, 2026-08-18** — delivered in `fb895df` (2026-08-17): +/-15s steppers on the upcoming rest (remembered per-exercise, floored at 5s) and +15s/-15s on the running countdown. Confirmed present on this branch. | **Corrected to built, 2026-08-18** — delivered in `fb895df` (2026-08-17): AudioContext beep primed on the "start rest" tap, `navigator.vibrate` silent-mode fallback, `alertedRef` guards one alert per countdown. Confirmed present on this branch. | **Corrected to built, 2026-08-18** — delivered in `4191575` (2026-08-17): `client_notes` table (migration `20260817_client_notes.sql`, applied to prod), `GET/POST /api/client-notes` + `DELETE /api/client-notes/[id]`, `NotesPanel` on the PWA profile, `ClientNotesPanel` on desktop. Confirmed present on this branch. |# Change Requests — Eternal Fitness Website
...
| CR-EF-033 | [BUG] Session dates differ between client-detail tab and mobile — tab renders `completed_at` only (`—` when unlogged), mobile renders `scheduled_at`; NULLS-first DESC ordering can push logged sessions out of the 50-row window | Bug | raised | 2026-08-17 | Assessment §1.3, fix H4. Timezone audited and ruled out. |
| CR-EF-034 | [BUG] Session naming inconsistent — `focus_label` ignored by client-detail tab (`Block n · Sn`) and session page; three independent archetype label maps; Outlook uses a fourth format | Bug | raised | 2026-08-17 | Assessment §1.4, fix H6 + §3.5 (one name, one label map). **Re-verified 2026-08-18, still unfixed.** `SessionWorkoutLog.tsx` (desktop session screen) has no `focus_label` usage anywhere and no first-class `sessions.status` column read or written -- `started_at` is still stamped on mount, the exact pattern this CR and CR-EF-030 both describe as the root cause. H6 not built. |
| CR-EF-035 | [BUG] Schedule→client navigation broken — `ScheduleCalendar.tsx:244` and the retired `/hub/log` redirect build `/hub/clients/<UUID>/…` while client routes resolve by `client_number` (`parseInt(uuid)` → NaN) | Bug | raised | 2026-08-17 | Assessment §1.8, fix H5. |
