...
| CR-EF-024 | Quick-action task on the client page | CR (functionality) | raised | 2026-08-17 | **Scope settled (Craig, 2026-08-17, `qmsxbh96xbu`): EF-local tasks only — no mirror into `decoded-ops-hub`.** Self-contained: new `client_tasks` table (client_id, text, due date, done_at) plus a quick-add control on the client profile. Usefully decouples this from the missing hub `project_id` noted under the process gap below — that blocker no longer applies here. Pairs naturally with CR-EF-017's notes surface (same place on the profile, same "capture it in one tap" intent) — worth designing the two together. |
