...
| CR-EF-023 | Update interval — allow an explicit date, or an arbitrary number of weeks | CR (functionality) | **built, migration not yet run** | 2026-08-17 | Verified: `lib/updates-due.ts:9` was a fixed enum (`4_week` / `6_week` / `12_week` / `6_session` / `flexible`) with `INTERVAL_DAYS` = 28/42/84. Built: two new interval modes, `custom_weeks` (N weeks from last send, same recurring pattern as the presets) and `fixed_date` (a date Esther pins directly, doesn't auto-advance). Code changes: `lib/updates-due.ts` (`computeUpdateDue` takes an `opts.weeks`/`opts.fixedDate`), `lib/updates-due-db.ts` (query widened + `LEFT JOIN` so a never-sent `fixed_date` client still surfaces), `UpdateIntervalControl.tsx` (weeks/date inputs shown conditionally), `page.tsx` (passes the new columns through), reminder email shows the actual week count. `npx tsc --noEmit` clean; UI verified live in the hub (both new dropdown options render correctly). **Requires `supabase/migrations/20260817_update_interval_custom.sql` to be run before the two new modes are selectable** — adds `update_interval_weeks`/`update_interval_next_date` columns and widens the `update_interval` CHECK constraint. Not run this session per this repo's standing convention that `clients`-table migrations need Craig's explicit per-session go-ahead (same header as `20260801_client_update_interval.sql`). |
...
| CR-EF-026 | [BUG] Colin and Saffron show PAR-Q not signed, but their PAR-Qs are signed | Bug | **root-caused 2026-08-17 — two different bugs, same symptom** | 2026-08-17 | Diagnosed live against production data (via the SSH-tunnelled hub, not `psql` — that's still not on PATH) by reading each client's own Document Register (`/hub/clients/[n]?tab=admin`) alongside `lib/compliance.ts:31-38`. The earlier `client_tracker` theory was wrong — checked and ruled out; the real causes are document-register display gaps, not that table. **Colin (#18):** his `client_documents` PAR-Q row is `status: "Sent"` — never signed through the document engine — yet his compliance badge shows **Clear, 0 outstanding**. Since `hasSignedParqDocument` is false and no "trainer-overridden" outstanding message appears (ruling out `parq_trainer_override`), the only remaining path in the `if` chain is a **legacy `signed_parq` row** satisfying compliance behind the scenes. That row is real and does its job for compliance gating — but the Document Register only ever lists `client_documents` rows, so it never surfaces it. Craig looking at Colin's Admin tab sees exactly one PAR-Q line, reading "Sent", and reasonably reads that as unsigned — compliance is quietly fine, but nothing on that screen tells him so. **Saffron (#12):** her compliance is genuinely satisfied — a signed, scanned-original PAR-Q document exists (`client_documents`, `status: "Signed"`, 15 Aug 2026) and correctly drives her Clear badge. But a second, older, never-completed PAR-Q row (`status: "Sent"`, 10 Jul 2026) still sits in the same register with no relationship shown between the two — unlike the Client Feedback document kind on the same page, which does get marked `Superseded` when replaced. A stale unsigned row sitting next to the real signed one is exactly the kind of thing a glance misreads as "not signed." **Two independent fixes, Craig's call on scope:** (1) surface the legacy `signed_parq` record in the Document Register (or migrate/backfill it into `client_documents` so the register becomes a true single source of truth — the FF doc already flags `signed_parq` as "not yet retired"); (2) mark a `client_documents` row `superseded` once a later signed row of the same `kind` exists for that client, matching the pattern already built for Client Feedback. Deliberately **not touched this session** — this reaches into compliance-gating logic (`do_not_train` safety gating) and the document engine, both beyond today's three scoped items, and there are two legitimate directions to fix it. Still belongs in the hub bug tracker once `project_id` is confirmed. |
