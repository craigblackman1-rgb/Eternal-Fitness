...
- 2026-08-15 (later, `add-workouts-training-block-f72141` worktree) — **G1 and G3
  answered by Craig directly, without waiting for L1's inventory pass:**
  - G1: Block = N sessions within a timeframe (6/12-week, 2–3×/week), the
    container. Session = one scheduled occurrence, one workout runs in it.
    Template = a reusable workout that runs in a session — content, not the
    container. Conversion story: template → assigned into a session; a
    session's actual content → saved back as a template.
  - G3: consolidate — one desktop logging screen replaces the Session Editor's
    inline logger and Standalone Live Log both. Train Screen (mobile) untouched.
  Recorded in full in `.context/brief-workout-consolidation-opendesign.md`, which
  is now unblocked and ready to hand to Open Design. L1 (full inventory + naming
  proposal) is effectively superseded for the naming question — G1 didn't need
  a proposal, Craig just answered it — but the rest of L1's inventory (mapping
  every workout-shaped component/data path) hasn't been done and still gates L2
  starting for real, per the WO's own dependency chain.
- 2026-08-15 (same worktree, later) — **L1 run in full** (Explore agent, verified
  against actual code, not assumed). Findings folded into
  `.context/brief-workout-consolidation-opendesign.md`: complete capability
  matrix across the 3 desktop/mobile surfaces (desktop today has neither
  warm-up flagging, idempotent writes, nor correctly-PB-gated logging — a real
  safety-relevant gap vs. mobile, flagged as a design decision not just a merge);
  the workout-templates browser at `/hub/workout-templates` already exists and
  works — L2's "genuinely new design" framing for it was wrong, it's a skin-only
  pass; `rescaleTemplateSection` referenced in this WO's L4 does not exist in the
  codebase; the complete (grep-verified) list of everything linking to
  `/hub/log/[sessionId]` that must keep working; two unrelated "templates"
  systems exist (`document_templates` at `/hub/templates` vs `workout_templates`
  at `/hub/workout-templates`) and must not be conflated; existing mockups
  `hub-session-editor.html` + `hub-session-log.html` should be merged, not
  replaced from scratch; the Mid-session Edit Sheet (mobile, unchanged) depends
  on the workout-templates API response shape staying compatible. **L2 (mockups)
  is now unblocked** — brief is ready to go to Open Design.
- 2026-08-15 (same worktree, later still) — Craig raised the in-app monthly
  calendar + session/workout decoupling requirement while scoping calendar sync
  (`.context/scope-of-works-2026-08-15.md` §2.1). Folded into the brief: the
  consolidated logger (desktop + PWA) must handle a session whose workout hasn't
  been defined yet — "build the workout now" as a first-class path, not an empty/
  broken view. PWA-first priority per Craig ("edit a workout on the fly" is a
  studio-floor, not desk-planning, need). One open architecture question flagged
  to the scope doc, not resolved here: whether a session can exist without a
  block at all (today `block_id NOT NULL`) — doesn't change what this brief's
  screens need to design for either way.
