...
**Mostly exists, one real gap.** `workout_templates` table (`supabase/migrations/20260802_workout_templates.sql`) already stores reusable session prescriptions with `condition_tags`, `archetypes`, `movement_type`, `muscle_groups`, `difficulty` — this is Esther's "condition-specific frameworks" ask, already built, not "Movement Slots" terminology but functionally the same shape (a template with swappable/taggable exercise data). `app/hub/(protected)/workout-templates/workout-template-browser.tsx` (365 lines) provides a browsing/filtering UI.
