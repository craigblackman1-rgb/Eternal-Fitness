// Creates the real "Rick — Block 1: Arrival to First Event" endurance_block
// document on Rick Frenken's actual production profile (client_number 23),
// populated from Rick_Block1_Plan_17Aug-5Sept2026.docx. This is the real
// deliverable, not a test doc -- do not delete after verification.
import fs from "fs";
import pg from "pg";
...
async function main() {
  ...
  const existing = await client.query(
    "SELECT id, title FROM client_documents WHERE kind = 'endurance_block' AND client_id = (SELECT id FROM clients WHERE client_number = 23)"
  );
  if (existing.rows.length > 0) {
    console.log("Already exists, not creating a duplicate:", existing.rows);
    await client.end();
    return;
  }

  const tmpl = await client.query("SELECT id, requires_client_signature, requires_trainer_signature FROM document_templates WHERE kind = 'endurance_block'");
  const clientRow = await client.query("SELECT id FROM clients WHERE client_number = 23");
  const t = tmpl.rows[0];
  const c = clientRow.rows[0];
  if (!t || !c) throw new Error("Missing template or client row");

  const body = { sections: [], enduranceBlock };
  const res = await client.query(
    `INSERT INTO client_documents (client_id, kind, title, template_id, template_version, body, requires_client_signature, requires_trainer_signature, status, version)
     VALUES ($1, 'endurance_block', $2, $3, 1, $4::jsonb, $5, $6, 'draft', 1) RETURNING id`,
    [c.id, "Rick — Block 1: Arrival to First Event", t.id, JSON.stringify(body), t.requires_client_signature, t.requires_trainer_signature],
  );
  console.log("Created on Rick's production profile:", res.rows[0].id);
  await client.end();
}
